...
namespace Discount.Grpc.Services
{
    public class DiscountService(DiscountContext dbContext, ILogger<DiscountService> logger) : DiscountProtoService.DiscountProtoServiceBase
    {
        ...
        public override async Task<CouponModel> CreateDiscount(CreateDiscountRequest request, ServerCallContext context)
        {
            ...
            var dbcoupon = await dbContext.Coupons.FirstOrDefaultAsync(x => x.ProductName == coupon.ProductName);
            if (dbcoupon is not null)
                throw new RpcException(new Status(StatusCode.NotFound, $"Discount with ProductName={coupon.ProductName} is already  exist"));
            coupon.Id = 0;
